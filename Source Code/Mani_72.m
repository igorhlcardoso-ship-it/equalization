...
for i=1:length(Es_N0)
    ...
    fig1 = figure ();
    scatter(real(trans_signal(1:F:end)), imag(trans_signal(1:F:end)), 10, 'filled'); grid on;
    title('Constellation of transmitted signal', 'FontSize', 12);
    xlabel('I');
    ylabel('Q');

    fig2 = figure();
    scatter (real(z_est), imag(z_est), 10, 'filled'); grid on;
    title('Constellation of received signal after sampling', 'FontSize', 12);
    xlabel('I');
    ylabel('Q');
 end 